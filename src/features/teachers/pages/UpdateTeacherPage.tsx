...
import { UPDATE_TEACHER_MUTATION } from "@/features/school/pages/teachers/services/teacherMutations";
import { TEACHER_QUERY } from "@/features/school/pages/teachers/services/teacherQueries";
import type {
...
export default function UpdateTeacherPage() {
  ...
  const { data: teacherData, loading: isLoadingTeacherData } = useQuery<
    TeacherQueryResponse,
    { teacherId: string }
  ...
  React.useEffect(() => {
    if (teacherData) {
      const { contactNumber, email, grades, name, school } =
        teacherData.teacher;
